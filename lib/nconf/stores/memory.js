...
//
Memory.prototype.merge = function (key, value, ignorePath) {
  if (this.readOnly) {
    return false;
  }
  ...
  var self    = this,
      target  = this.store,
      path    = ignorePath ? [key] : common.path(key, this.logicalSeparator),
      fullKey = key;
  ...
  return Object.keys(value).every(function (nested) {
    return self.merge(common.keyed(self.logicalSeparator, fullKey, nested), value[nested], ignorePath);
  });
};
